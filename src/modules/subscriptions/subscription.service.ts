...
const THROTTLE_DURATION_S = Math.ceil(
   envConfig.SSE_THROTTLE_DURATION_MS / 1000
);
...
export async function createSubscription(
   ...
   if (!redis) {
      throw new Error(
         'Redis is unavailable; subscriptions require a Redis connection'
      );
   }
...
export async function getWalletSubscriptions(
   ...
   const redis = getRedis();
   if (!redis) return [];
   const ids = await redis.zrange(walletSubsKey(walletAddress), '0', '-1');
