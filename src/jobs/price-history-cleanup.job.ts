...
function assertSafeTableName(tableName: string) {
   if (!VALID_IDENTIFIER.test(tableName)) {
      throw new Error(
         `Invalid PRICE_HISTORY_TABLE_NAME: ${tableName}. Expected a simple SQL identifier.`
      );
   }
}
...
/**
 * Delete price history rows older than the retention window.
 *
 * The history table grows with every trade (one row per event, by design — the
 * API uses it for TWAP and range queries), so without this job it grows without
 * bound. Rows are pruned by `recordedAt`, which is indexed for exactly this
 * query. Set PRICE_HISTORY_CLEANUP_DRY_RUN=false to actually delete.
 */
export async function cleanupExpiredPriceHistory(
   ...
   const tableName = envConfig.PRICE_HISTORY_TABLE_NAME;
   const dryRun = envConfig.PRICE_HISTORY_CLEANUP_DRY_RUN;
   const cutoffTimestamp = getCutoffTimestamp(
      now(),
      envConfig.PRICE_HISTORY_RETENTION_DAYS
   );
   ...
   if (!exists) {
      logger.warn(
         ...
         'Price history cleanup skipped because target table was not found'
      );
   ...
   if (dryRun) {
      const rows = await queryRawUnsafe<Array<{ count: number }>>(
         `SELECT COUNT(*)::int AS count FROM "${tableName}" WHERE "recordedAt" < $1`,
         cutoffTimestamp
      );
      ...
      logger.info(
         {
            tableName,
            cutoffTimestamp: cutoffTimestamp.toISOString(),
            retainedDays: envConfig.PRICE_HISTORY_RETENTION_DAYS,
            wouldDeleteCount: count,
            dryRun,
         },
         'Price history cleanup dry-run completed'
      );
   ...
   const deletedCount = await executeRawUnsafe(
      `DELETE FROM "${tableName}" WHERE "recordedAt" < $1`,
      cutoffTimestamp
   );
...
export function startPriceHistoryCleanupJob() {
   ...
   const intervalMs =
      envConfig.PRICE_HISTORY_CLEANUP_INTERVAL_MINUTES * 60 * 1000;
   ...
   logger.info(
      {
         ...
         dryRun: envConfig.PRICE_HISTORY_CLEANUP_DRY_RUN,
         tableName: envConfig.PRICE_HISTORY_TABLE_NAME,
      },
